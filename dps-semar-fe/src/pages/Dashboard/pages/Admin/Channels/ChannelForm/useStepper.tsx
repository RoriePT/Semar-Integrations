...
const useStepper = (
  ...
  const handleFormValidated = async () => {
    ...
    const payload: ChannelRequestDtoType = getPayload(
      tab1State,
      tab2State,
      !!editData
    );
    ...
    let response = null;
    if (!editData) response = await RegisterAPIs.registerChannel(payload);
    else response = await RegisterAPIs.updateChannel(payload, editData.id);
