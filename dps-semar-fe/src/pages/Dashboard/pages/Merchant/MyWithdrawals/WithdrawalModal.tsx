...
const WithdrawalModal = ({ opened, onClose, reload }) => {
  ...
  const [channelProfiles, setChannelProfiles] = useState([
    { value: "upi", label: "UPI", disabled: false },
    { value: "eWallet", label: "E-Wallet", disabled: false },
    { value: "netBanking", label: "Net Banking", disabled: false },
  ]);
  ...
  const handleSuccess = async () => {
    try {
      ...
      setError((prev) => ({ ...prev, amount: "Failed to create withdrawal order" }));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      >
        ...
        <Button onClick={handleSubmit} mt="md" loading={loading}>
