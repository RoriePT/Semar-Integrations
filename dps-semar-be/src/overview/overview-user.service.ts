...
export class OverviewUserService {
  ...
  async getUpiVendorOverviewDetails(userId) {
    const upiVendor = await this.upiVendorRepository.findOne({
      where: { id: userId },
      relations: ['identity', 'identity.upi'],
    });
    if (!upiVendor) throw new NotFoundException('UPI Vendor not found!');

    // Get all UPI IDs with their settlement amounts
    const upiIds = upiVendor.identity.upi
      .filter((upi) => upi.isUpiVendor)
      .map((upi) => ({
        id: upi.id,
        upiId: upi.upiId,
        title: upi.title,
    ...
    // Calculate total settlement across all UPI IDs
    const totalSettlement = upiIds.reduce(
      (sum, upi) => sum + upi.settlementAmount,
      0,
    );

    // Get payin orders for this UPI vendor
    const payinRows = await this.payinRepository.find({
      where: {
        upiVendor: { id: upiVendor.id },
      },
      relations: ['upiVendor'],
    });

    // Count payins and calculate total amounts
    const payins = payinRows.reduce(
      (prev, curr) => {
        if (curr.status === OrderStatus.COMPLETE) {
          prev.completed++;
          prev.completedAmount += curr.amount;
        } else if (curr.status === OrderStatus.SUBMITTED) {
          prev.pending++;
          prev.pendingAmount += curr.amount;
        } else if (curr.status === OrderStatus.ASSIGNED) {
          prev.pending++;
          prev.pendingAmount += curr.amount;
        }
        return prev;
    ...
    // Get commission from transaction updates
    // Note: UPI vendor commission entries don't have a user relation, so we join only on payinOrder
    // Filter by order status COMPLETE - if order is complete, commission should be counted
    const allUpiVendorCommissions = await this.transactionUpdateRepository.find(
      {
        where: {
        userType: UserTypeForTransactionUpdates.UPI_VENDOR_COMMISSION,
        },
        relations: ['payinOrder', 'payinOrder.upiVendor'],
      },
    );

    // Filter for this specific vendor and completed orders
    const completedCommissions = allUpiVendorCommissions.filter(
      (entry) =>
        entry.payinOrder?.upiVendor?.id === upiVendor.id &&
        entry.payinOrder?.status === OrderStatus.COMPLETE,
    );

    const totalCommissionEarned = completedCommissions.reduce((prev, curr) => {
      return prev + (curr.amount || 0);
    }, 0);

    // Monthly commission graph for last 5 months
    const currentMonthIndex = moment().month();
    const monthsOfYear = monthNames();
    const monthlyCommissions = [];
    ...
    completedCommissions.forEach((transaction) => {
      const month = moment(transaction.createdAt).month();

      for (let i = 0; i < 5; i++) {
        const monthIndex = (currentMonthIndex - i + 12) % 12;
        if (month === monthIndex) {
          monthlyCommissions[4 - i].commission += transaction.amount || 0;
          break;
        }
      }
    });

    // Round commission values in monthly data
    monthlyCommissions.forEach((data) => {
      data.commission = roundOffAmount(data.commission);
    });

    return {
      upiIds: upiIds,
      totalSettlement: roundOffAmount(totalSettlement),
      payins: {
        ordersPending: payins.pending,
        ordersCompleted: payins.completed,
        pendingAmount: roundOffAmount(payins.pendingAmount),
        completedAmount: roundOffAmount(payins.completedAmount),
        totalAmount: roundOffAmount(
          payins.pendingAmount + payins.completedAmount,
        ),
      },
      commission: {
        total: roundOffAmount(totalCommissionEarned),
