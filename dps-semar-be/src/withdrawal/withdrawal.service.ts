...
export class WithdrawalService {
  ...
  async create(createWithdrawalDto: CreateWithdrawalDto, email) {
    ...
    // Get withdrawal service rate for the user
    const withdrawalServiceRate = isMerchant
      ? user.merchant?.withdrawalServiceRate
      : user.agent?.withdrawalRate || 0;

    // Calculate service fee
    const serviceFee = (withdrawalAmount / 100) * withdrawalServiceRate;
    ...
    // Check if withdrawal amount + service fee exceeds balance
    if (currentAvailableBalance < totalDeduction)
      return {
        error: true,
        message: `Insufficient balance! Withdrawal amount (₹${withdrawalAmount}) + service fee (₹${roundOffAmount(serviceFee)}) = ₹${roundOffAmount(totalDeduction)} exceeds your available balance (₹${currentAvailableBalance}).`,
      };
