...
export class RolesGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    // Check if Role.ALL is in the required roles (case-insensitive)
    if (
      requiredRoles &&
      requiredRoles.some((role) => role?.toLowerCase() === 'all')
    )
      return true;
    ...
    if (verifiedToken) {
      ...
      switch (userType) {
        ...
        case Role.UPI_VENDOR:
          const upiVendor = this.entityManager.findOne(UpiVendor, {
            where: {
              id: verifiedToken.id,
            },
          });

          if (!upiVendor) throw new ForbiddenException('UPI Vendor not found');
          break;
    ...
    if (verifiedToken?.type) {
      ...
      switch (verifiedToken.type.toLowerCase()) {
        ...
        case Role.UPI_VENDOR:
          user = await this.entityManager.findOne(UpiVendor, {
            where: { id: verifiedToken.id },
          });
