...
export class BulletinService {
  constructor(
    ...
    private upiVendorService: UpiVendorService,
  ) {}
  ...
  async getUpiVendorPendingOrders(
    vendorId: number,
    paginateDto: PaginateRequestDto,
  ) {
    return await this.upiVendorService.getPendingOrders(vendorId, paginateDto);
  }

  async getDetailsForSettlement(vendorId: number) {
    return await this.upiVendorService.getDetailsForSettlement(vendorId);
  }
}
