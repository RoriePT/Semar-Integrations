...
export class PayoutMemberService {
  ...
  async getPayoutDetails(id: string) {
    ...
    const userChannelName = orderDetails.channel;
    ...
    if (
      ...
      const upiId = orderDetails.user.upiDetails.upiId;
      ...
      transactionDetails = {
        ...
        recipient: {
          ...parseUserChannelDetails(orderDetails?.user),
      ...
      transactionDetails = {
        transactionId: orderDetails.transactionId,
        receipt: orderDetails.transactionReceipt,
        recipient: parseUserChannelDetails(orderDetails?.user),
        member: orderDetails.member
          ? JSON.parse(orderDetails.transactionDetails)
          : null,
        gateway: orderDetails.gatewayName
