...
export class PayoutService {
  ...
  async create(payoutDetailsDto: CreatePayoutDto, merchantId, ip: string) {
    const {
      orderId,
      user,
      paymentMethod,
    ...
    if (existingPayoutWithSameOrderId)
      throw new BadRequestException(
        'An order with this orderId already exists!',
      );

    const channelConfig = await this.channelService.getConfig(paymentMethod);

    if (channelConfig.outgoing === false)
      throw new NotAcceptableException(
        `Payment method ${paymentMethod} is not available currently.`,
      );
    ...
    if (!merchant.enablePayouts)
      throw new ForbiddenException(
        'Payouts are currently not enabled for your account!',
      );
    ...
    if (amount < merchant.minPayout || amount > merchant.maxPayout)
      return {
        error: true,
    ...
    let endUserData = await this.endUserRepository.findOne({
      where: { userId: user.id, merchant: { id: merchantId } },
    });
    ...
    if (endUserData) {
      ...
      await this.endUserRepository.update(endUserData.id, {
        ...
        email: user.email,
        upiDetails:
          paymentMethod === ChannelName.UPI
            ? upiDetails
            : endUserData.upiDetails,
        eWalletDetails:
          paymentMethod === ChannelName.E_WALLET
            ? eWalletDetails
            : endUserData.eWalletDetails,
        netBankingDetails:
          paymentMethod === ChannelName.BANKING
            ? netBankingDetails
            : endUserData.netBankingDetails,
      });
    } else {
      endUserData = await this.endUserService.create({
        ...
        userId: user.id,
        merchant,
        upiDetails: paymentMethod === ChannelName.UPI ? upiDetails : null,
        eWalletDetails:
          paymentMethod === ChannelName.E_WALLET ? eWalletDetails : null,
        netBankingDetails:
          paymentMethod === ChannelName.BANKING ? netBankingDetails : null,
      });
    }

    const payout = await this.payoutRepository.save({
      merchantOrderId: orderId,
      amount: payoutDetailsDto.amount,
      user: endUserData,
      merchant,
      systemOrderId: `PAYOUT-${uniqid()}`.toUpperCase(),
      channel: payoutDetailsDto.paymentMethod,
      webhookUrl: payoutDetailsDto?.webhookUrl || null,
    });
    ...
    if (this.enableGateway) {
      ...
      const timeoutId = setTimeout(async () => {
        ...
        const result = await this.paymentSystemService.makeGatewayPayout({
          ...
          amount: payout.amount,
          orderType: OrderType.PAYOUT,
          mode: paymentMethod === ChannelName.BANKING ? 'imps' : 'upi',
        ...
        if (result)
          await this.updatePayoutStatusToAssigned({
            ...
            gatewayServiceRate: payout?.gatewayServiceRate || 0,
            gatewayName: result?.gatewayName,
          });

        if (result?.paymentStatus || result?.transactionId) {
    ...
    return {
      ...
      data: {
        orderId: payout.merchantOrderId,
        kgOrderId: payout.systemOrderId,
        status: payout.status,
        message:
          'Your payout order has been initiated successfully. The payment is being processed and may take some time to complete. You can check the current status using the Get Payout Order Status API.',
      },
    };
  }
  ...
  async updatePayoutStatusToAssigned(
    ...
    if (memberData && payoutOrderData.payoutMadeVia === PaymentMadeOn.MEMBER) {
    ...
    if (
      ...
      payoutOrderDetails.status !== OrderStatus.INITIATED
    )
    ...
    if (paymentMode === PaymentMadeOn.GATEWAY) {
      await this.transactionUpdatePayoutService.create({
        orderDetails: payoutOrderDetails,
        userId: payoutOrderDetails.merchant?.identity?.id,
        forGateway: true,
        gatewayServiceRate,
        orderType: OrderType.PAYOUT,
        systemOrderId: payoutOrderDetails.systemOrderId,
  ...
  async updatePayoutStatusToComplete(body) {
    ...
    this.callPayoutWebhooksForApiMode(id);
  ...
  async updatePayoutStatusToFailed(body) {
    ...
    if (!forExpire && payoutOrderDetails.status !== OrderStatus.SUBMITTED)
      throw new NotAcceptableException(
        'order status is not submitted or already failed or completed!',
      );
  ...
  async updatePayoutStatusToSubmitted(body) {
    ...
    if (
      payoutOrderDetails.payoutMadeVia !== PaymentMadeOn.GATEWAY &&
      payoutOrderDetails.status !== OrderStatus.ASSIGNED
    )
  ...
  async getOrderStatusForApiMode(orderId: string, merchantId: number) {
    ...
    return {
      ...
      data: {
        ...
        transactionDetails: {
          ...
          amount: payout.amount,
          paymentMethod: payout.channel,
          time: payout.updatedAt,
        },
      },
    };
  }

  async callPayoutWebhooksForApiMode(orderId: string) {
    const payout = await this.payoutRepository.findOne({
      where: {
        systemOrderId: orderId,
      },
    ...
    const webhookUrl = payout?.webhookUrl;
    if (!webhookUrl) return;

    const tokenPayload = {
      iss: process.env.BASE_URL,
      sub: payout.merchant.integrationId,
      order: payout.merchantOrderId,
      kgOrder: payout.systemOrderId,
      utr: payout?.utr,
    };

    const token = await this.jwtService.createTokenForWebhookVerification(
      tokenPayload,
      payout.merchant?.apiKey,
    );
    ...
    try {
      ...
      if (res) await this.handleNotificationStatusSuccess(payout.systemOrderId);
    } catch (error) {
      console.error(`Error calling webhook URL ${webhookUrl}`, error);
    }
  }

  async updatePayoutStatusManual(
    ...
    if (payout.status === OrderStatus.INITIATED)
      await this.updatePayoutStatusToAssigned({
        id: payout.systemOrderId,
        paymentMode: PaymentMadeOn.GATEWAY,
        gatewayServiceRate: 0,
        gatewayName: GatewayName.MANUAL,
      });

    if (payout.status === OrderStatus.ASSIGNED)
      await this.updatePayoutStatusToSubmitted({
        id: payout.systemOrderId,
        transactionId: `ADMIN MANUAL ADJUSTMENT`,
        transactionReceipt: 'ADMIN ADJUSTMENT',
        transactionDetails: {
          date: new Date(),
        },
      });
    ...
    if (status === OrderStatus.COMPLETE)
      await this.updatePayoutStatusToComplete({ id: payout.systemOrderId });

    if (status === OrderStatus.FAILED)
      await this.updatePayoutStatusToFailed({ id: payout.systemOrderId });

    return HttpStatus.OK;
  }

  async getOldIncompletedOrders() {
    const expiryTime = new Date(Date.now() - 25 * 60 * 60 * 1000);
