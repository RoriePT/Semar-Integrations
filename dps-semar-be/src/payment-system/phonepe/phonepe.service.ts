...
export class PhonepeService {
  ...
  private async getCredentials(environment) {
    ...
    if (!decryptedMerchantId || !decryptedSaltIndex || !decryptedSaltKey) {
      console.log('Failed to decrypt Phonepe keys');
      return;
    }
  ...
  async getPayPage(getPayPageDto: GetPayPageDto) {
    ...
    const payload = {
      ...
      redirectUrl: `${process.env.PAYMENT_PAGE_BASE_URL}/gateway-callback?orderId=${orderId}&environment=${environment}`,
  ...
  async getPaymentStatus(transactionId: string, environment) {
    ...
    if (transactionId) {
      ...
      try {
        ...
        let status = '';
        if (response.data?.data?.responseCode === 'REQUEST_CANCEL_BY_REQUESTER')
          status = 'FAILED';

        return {
          status: status || response.data?.data?.responseCode,
          details: {
  ...
  async receivePhonepeRequest(req, body, environment) {
    ...
    try {
      ...
      if (!receivedXVerify) {
        console.log('Missing X-VERIFY header');
        return;
      }
      ...
      if (!base64Response) {
        console.log('Missing response field in request body');
        return;
      }
      ...
      if (receivedXVerify !== xVerifyCheckSum) {
        console.log('Invalid X-VERIFY header');
        return;
      }
