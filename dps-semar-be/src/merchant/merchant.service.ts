...
export class MerchantService {
  ...
  async create(createMerchantDto: CreateMerchantDto) {
    ...
    const merchant = this.merchantRepository.create({
      ...
      agentCommissions: agentId
        ...
        : null,
      upiVendorAutoVerifyThreshold: upiVendorAutoVerifyThreshold || null,
    });
  ...
  async update(id: number, updateDto: UpdateMerchantDto) {
    ...
    delete updateDto.integrationId;
    delete updateDto.apiKey;
  ...
  async authenticate(
    authenticateRequestBody: AuthenticateRequestDto,
    ip: string,
  ) {
    const { api_key, integration_id } = authenticateRequestBody;
    ...
    try {
      ...
      if (!merchant)
        throw new NotFoundException(
          `Merchant with ${integration_id} integration ID not found!`,
        );

      if (merchant.apiKey !== api_key)
        throw new UnauthorizedException(
          'Invalid or incorrect API key provided!',
        );
      ...
      const accessToken = this.jwtService.createToken(
        {
          ...
          type: merchant.identity.userType,
        },
        '2d',
      );
  ...
  async getBalance(merchantId: number) {
    const merchant = await this.merchantRepository.findOne({
      where: {
        id: merchantId,
      },
  ...
  async getChannelConfig(merchantId: number) {
    const merchant = await this.merchantRepository.findOne({
      where: { id: merchantId },
    });
    ...
    const parsedPayinChannels = JSON.parse(merchant.payinChannels);
    const parsedPayoutChannels = JSON.parse(merchant.payoutChannels);

    const channels = ['UPI', 'E_WALLET', 'NET_BANKING'];

    const payins = {};
    const payouts = {};

    for (const channel of channels) {
      const config = await this.channelService.getConfig(
        channel as ChannelName,
      );

      payins[channel] =
        config?.incoming && parsedPayinChannels.includes(channel);
      payouts[channel] =
        config?.outgoing && parsedPayoutChannels.includes(channel);
    }
  ...
  async getBasicDetailsByIntegrationId(integrationId: string) {
    const merchant = await this.merchantRepository.findOne({
      where: { integrationId },
      select: ['enableUpiVendorGateway'],
    });

    if (!merchant)
      throw new NotFoundException(
        `Merchant with integration ID ${integrationId} not found!`,
      );

    return {
      status: HttpStatus.OK,
      data: {
        enableUpiVendorGateway: merchant.enableUpiVendorGateway,
      },
    };
  }
}
